(function () {
  ...
  */
  ...
  function createCircles() {
    ...
    */
    const quantity = d3.select("#quantity").property('value')
  ...
  function deleteCircles() {
    ...
    */
    ...
    var deleteConfirmation = confirm("Do you want to delete all circles ?");
  ...
  function textTip(radius, position, color) {
     ...
     */
    let label = function(label) { return "<span class='label'>" + label + ": </span>" }
    const br = " <br>" 
    const r = label("Radius") + parseInt(radius) + br 
    const p = label("Coord") + "( "+ parseInt(position.x) + ", " + parseInt(position.y) +" )"+ br
    const c = label("Color") + color
    return r+p+c;
  }
